...
class AdslDownLoader(object):
    ...
    def get_body(self, header, body):
        if ('Content-Type' in header and self.re_image_content_type.search(header['Content-Type'])) or                 ('content-type' in header and self.re_image_content_type.search(header['content-type'])):
            return base64.b64encode(body)

        elif ('Content-Encoding' in header and header['Content-Encoding']) or                 ('content-encoding' in header and header['content-encoding']):
            import gzip
            import StringIO
