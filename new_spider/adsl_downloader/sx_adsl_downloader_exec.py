...
class executable_file(object):

    def __init__(self):
        pass

    def save_file(self, code_str):
        file = 'test.py'
        f = open(file, 'w')
        f.write(code_str)
        f.close()
    ...
    def excute_file(self, file):
        ...
        # print return_str
        readfile = os.popen("python test.py")
        sx = readfile.read()
        readfile.close()
        print sx
...
def test(code_str):
    excutor = executable_file()
    excutor.excute_file(excutor.save_file(code_str))
