...
class AdslDownLoader(object):
    ...
    def run_test(self):
        ...
        for task in self.tasks:
            ...
            # if extractor_html != "":
                ...
                # for extractor in extractors:
            self.get_result(opener, request, result, redirect, True)
            redirect_u = self.extractor_html(result["result"])
            request = urllib2.Request(redirect_u)
            redirect_handler = UnRedirectHandler()
            opener = urllib2.build_opener(redirect_handler)
            self.get_result(opener, request, result, redirect, True)
    ...
    def extractor_html(self, body):
        ...
        return "http://xin.baidu.com/detail/basicAjax?pid="+ pid +"&tot="+ tot
...
def main():
    ...
    downloader.run()
    downloader.tasks = [{"id": 1, "url": "http://xin.baidu.com/detail/compinfo?pid=PbObuEVtIKUUFMyW7w*MAHcEPPjWmHT5lg3G&from=koubei",
                         "type": 1, "store_type": 1, "md5": "jkdjsjksdjds"}]
    ...
    # # #             "iPage": 2}
    # #
    # request = urllib2.Request("https://koubei.baidu.com/s/1a604f1c0e06a6cba8d4b995801b2714?fr=search",
    #             data=None, headers={})
    # redirect = "1"
    # redirect_handler = UnRedirectHandler()
    # opener = urllib2.build_opener(redirect_handler)
