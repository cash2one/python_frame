...
class EngineSogouSourceSpider(BaseSpiderSeparateDealAndStore):

    def __init__(self):
        super(EngineSogouSourceSpider, self).__init__()
	...
	try:
            r = self.basicExtractor.get_sogou_site(html)
	    res ={}
	    if r['domain_status'] == 1 and r['response_status'] == 1:
	        res['id'] = url['id']
                res['sl_num_sogou'] = r['sl_num_sogou']
                self.store_queue.put({'data': res, 'type':1, 'storeIndex': 0})
        except:
            return False
def Main():
    spider = EngineSogouSourceSpider()
