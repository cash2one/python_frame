...
class AizhanBasicSourceSpider(BaseSpiderSeparateDealAndStore):
    ...
    def get_results(self):
        ...
        sql = 'SELECT id,url from site_basicinfo where spidertype=%s' % spidertype
    ...
    def start_requests(self):
	    ...
	    results = self.get_results()
            ...
            for result in results:
                ...
                urls.append({'url': rank_url,'init':dm,'type': 1,'id':result[0]})
                url_len -=1
                    ...
                    if str(result['status']) == '2':
                        ...
                        if req_type == 2:
                            self.deal_aizhan_basic_response(u, result['result'], stores)
                             ...
                             check = self.deal_aizhan_pr_response(u, result['result'], stores)
                        elif req_type == 4:
                            check = self.deal_aizhan_br_response(u, result['result'], stores)
                        elif req_type == 5:
                            check =self.deal_aizhan_whois_response(u, result['result'], stores)
                        elif req_type == 6:
                            check = self.deal_aizhan_dns_response(u, result['result'], stores)
                        elif req_type == 7:
                            check = self.deal_aizhan_speed_response(u, result['result'], stores)
                        result = None
                        if check:
                            failed_urls.append({'url': r_url, 'type': 1,'init':u['init'],'id':u['id']})
...
def Main():
    ...
    spider.run(5, 50, 100, 50, 600, 600, 600, 600,True)
