...
class EngineSoSourceSpider(BaseSpiderSeparateDealAndStore):

    def __init__(self):
        super(EngineSoSourceSpider, self).__init__()
	...
	try:
            r = self.basicExtractor.get_so_site(html)
	    res ={}
	    if r['domain_status'] == 1 and r['response_status'] == 1:
	        res['id'] = url['id']
                res['sl_num_so'] = r['sl_num_so']
	        self.store_queue.put({'data': res, 'type':1, 'storeIndex': 0})
        except:
            return False
def Main():
    spider = EngineSoSourceSpider()
    spider.run(10, 30, 100, 50, 600, 600, 600, 600,True)
