...
class Scheduler(object):
    ...
    def __push_task(self, db, rds, user_id, priority, task_types, task_size, task_queue_key,  district_id, single=False, concurrent=False):
        ...
        try:
            ...
            if len(ids) > 0:
                ...
                self.log.info('push %d tasks' % len(ids))
            return len(ids)
        except Exception, e:
            self.log.error(traceback.format_exc())
        return 0
    ...
    def get_district_queue(self):
        '''
            获取地域队列  district_queue
        :return:
        '''
        district_queue = Queue()
        try:
            db = StoreMysql(**self.dbParam)
            ...
            if len(districts) != 0:
                for district in districts:
                    district_queue.put(district[0])
            return district_queue
        except Exception, e:
            self.log.error(traceback.format_exc())

    def district_TaskorNone(self, db, user_id):
        try:
            urls_table = self.userConfigs['urlsTable'] % int(user_id)
    ...
    def push(self, task_type):
        ...
        # adsl_task_size = self.configs['taskQueueAdslSize']
        max_size = 0
        ...
        while True:
            ...
            while weight_queue.qsize() > 0:
                ...
                if rds.llen(task_queue_key) < max_size:
                    ...
                    # 地域性
                    district_queue = self.get_district_queue()
                    # 查看是否有 地域性任务
                    district_flag = self.district_TaskorNone(db, user_id)
                    if district_flag:
                        while district_queue.qsize() > 0:
                            adsl_task_size = 0
                            for s in adsl_task_max_send_num:
                                adsl_task_size += adsl_task_max_send_num[s]

                            district_id = district_queue.get()
