    ...
    def __push_task(self, db, rds, user_id, priority, task_types, task_size, task_queue_key, district_id, single=False, concurrent=False):
        ...
        try:
            ...
            if len(ids) > 0:
                ...
                self.log.info('%s priority:%d push %d tasks ;%s' % (urls_table, priority, len(ids), str(datetime.now())))
            return len(ids)
        except Exception, e:
                            ...
                            for district_id in self.district_list:
                                ...
                                if rds.llen(district_task_queue) < adsl_task_size:
                                    ...
                                    while priority > 0:
                                        ...
                                        size = self.__push_task(db, rds, user_id, priority, task_types_in_database,
                                                                adsl_task_size, district_task_queue, district_id,
                                                                single,
                                                                concurrent)
                                        sended_size += size
                    ...
                    for result in results:
                        ...
                        if 'store_type' in result and int(result['store_type']) == 2:
                            ...
                            val = {'url': result['url'], 'status': result['status'], 'result': result['result'],
                                   'header': result['header'], 'redirect_url': redirect_url,
                                   'code': code, 'md5': result['md5'], 'id': url_id}
                            ...
                            sql = 'update %s set status = %s, result = "%s", header = "%s", ' \
                                  ...
                                  % (urls_table, result['status'], MySQLdb.escape_string(res),
                                     MySQLdb.escape_string(result['header']), redirect_url,
                                     code, url_id)

                        r = db.do(sql)
                        if r == -1:
                            # print "user_id :%s;url_id:%s" % (user_id, url_id)
        ...
        while True:
            try:
                for user in users:
                    ...
                    expire_time = str(datetime.now() + timedelta(seconds=-reset_interval))
...
def main():
    scheduler = Scheduler(config.TASK_TYPE_CAPTURE)
    scheduler.run(1, 0, 0, 30, 1, 0, 0)
