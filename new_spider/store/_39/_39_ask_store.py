...
class _39AskStore(SpiderStore):
    ...
    def store(self, results, table, unique=None, type=1, field=None):
        """
        将一个数组存储到指定的存储媒介中
        Args:
                ...
                try:
                    ...
                    if type == 1:
                        if unique is not None:
                            print 'select id from %s where %s = "%s"' % (table, unique, result[unique])
                            l = db.query('select id from %s where %s = "%s"' % (table, unique, result[unique]))
                            if len(l) == 0:
