...
class _39AskListStore(SpiderStore):
    """
    39问答列表存储器
    Attributes:
        ...
        if len(results) > 0:
            ...
            for result in results:
                try:
                    ...
                    if type == 1:
                        l = db.query('select id from health_ask_list where url = "%s"' % result['url'])
                        if len(l) == 0:
                            db.save('health_ask_list', result)
                    elif type == 2:
                        db.update('health_ask_list', result, field)
                except Exception, e:
                    pass
