...
class _39DiseaseListStore(SpiderStore):
    """
    39健康疾病列表页存储器
    Attributes:
        ...
        if len(results) > 0:
            ...
            for result in results:
                try:
                    ...
                    if type == 1:
                        l = db.query('select id from health_disease_list where url = "%s"' % result['url'])
                        if len(l) == 0:
                            db.save('health_disease_list', result)
                    elif type == 2:
                        db.update('health_disease_list', result, field)
                except Exception, e:
                    pass
