...
class _39DepartmentStore(SpiderStore):
    """
    39健康科室存储器
    Attributes:
        ...
        if len(results) > 0:
            ...
            for result in results:
                try:
                    ...
                    if type == 1:
                        l = db.query('select id from health_departments where department_id = "%s"' % result['department_id'])
                        if len(l) == 0:
                            db.save('health_departments', result)
                    elif type == 2:
                        db.update('health_departments', result, field)
                except Exception, e:
                    pass
