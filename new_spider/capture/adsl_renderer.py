...
class WebRender(QWebPage):
    ...
    def load_task(self, task):
        ...
        if 'data' in task:
            ...
            data.append(urlencode(json.loads(task['data'])).encode('ascii'))
        if self.type == 4:
        ...
        try:
            ...
            if url.isEmpty():
                ...
                if self.type == 4:
                    ...
                    self.result = base64.encode(img_data.toBase64().data().decode('utf-8')+"||||"+frame.toHtml())
                else:
                    self.result = base64.encode(frame.toHtml())
        except Exception:
            print('抓取失败')
        self.app.quit()
